...
description: Run Music Menu in a throwaway nested GNOME Shell, mirrored live on the user's desktop — click through it, screenshot it, then shut it down. Use whenever a change must be SEEN (layout, spacing, colour, animation end-states, the tabs, Library → Detail navigation, the overview and workspace-slide clones), or needs a fresh shell start (extension.js, metadata.json, a new UUID).
---

# Driving Music Menu in a nested shell

Music Menu renders onto the desktop background, so the only way to verify a visual
...
./scripts/nested.sh start               # ~2 s; Music Menu is ACTIVE when it returns
...
- a shell started from a Claude Code session stops itself after 10 minutes with no
  `nested.sh` command (`MUSIC_MENU_NESTED_IDLE=<seconds>` at `start`, `0` = never);
...
--all` shows everything. `[Music Menu]` lines are the extension's own.
...
- **dconf is shared with the real session, and other projects' nested shells
  ...
  database of its own (`music_menu_nested`, a writable layer that starts
  empty every time over a read-only one with Music Menu alone enabled and
...
- **`start` enables Music Menu** if dconf doesn't list it — which writes
...
- **Other extensions load too** (the nested shell reads the same extension list), so
  their log lines and top-bar icons appear alongside Music Menu.
- **The mirror needs GStreamer's PipeWire plugin.** If `mirror on` fails, use
  `start --headless` and screenshots, and tell the user.
- **Driving the prefs window:** `./scripts/nested.sh run gnome-extensions prefs music-menu@jackt &`
  ...
  process whose environment has `WAYLAND_DISPLAY=music-menu-dev`, never a
