...
//
//   local   ~/.local/share/music-menu/watched.json
//           every mark made on this machine, keyed by absolute path
//   folder  <library folder>/.music-menu-watched.json
...
const FOLDER_FILE = '.music-menu-watched.json';
const VERSION = 1;

function localPath() {
    return GLib.build_filenamev([GLib.get_user_data_dir(), 'music-menu', 'watched.json']);
}
...
export class Tracker extends Signals.EventEmitter {
    ...
    _loadLocal() {
        ...
        try {
            ...
            console.warn(`[Music Menu] Could not read ${path}: ${e}`);
        }
    }
    ...
    _saveLocal() {
        ...
        try {
            ...
            console.error(`[Music Menu] Could not write ${path}: ${e}`);
        }
    }
    ...
    _readFolder(folder) {
        ...
        file.load_contents_async(this._cancellable, (_file, result) => {
            ...
            try {
                ...
                if (!isNotFound(e)) {
                    console.warn(`[Music Menu] Could not read ${file.get_path()}: ${e.message}`);
                    return;
                }
            }
            if (this._mode !== 'source')
        ...
        file.replace_contents_bytes_async(bytes, null, false, Gio.FileCreateFlags.NONE,
            this._cancellable, (_file, result) => {
                ...
                try {
                    ...
                    if (e.matches?.(Gio.IOErrorEnum, Gio.IOErrorEnum.CANCELLED))
                        return;
                    console.warn(`[Music Menu] Could not write ${file.get_path()}: ${e.message}`);
                }
            ...
            file.delete_async(GLib.PRIORITY_DEFAULT, this._cancellable, (_file, result) => {
                try {
                    ...
                    if (!isNotFound(e) && !e.matches?.(Gio.IOErrorEnum, Gio.IOErrorEnum.CANCELLED))
                        console.warn(`[Music Menu] Could not remove ${file.get_path()}: ${e.message}`);
                }
            });
        }
        this._written.clear();
