...
export default class MusicMenuPreferences extends ExtensionPreferences {
    ...
    _readKeyDrop(path) {
        try {
            ...
            console.warn(`[Music Menu] Could not read ${path}: ${e.message}`);
            return '';
        }
    }
            ...
            try {
                ...
                proc.communicate_utf8_async(null, null, (p, result) => {
                    ...
                    try {
                        ...
                        if (failed)
                            console.error(`[Music Menu] Scan failed: ${stderr}`);
                    } catch (e) {
                        failed = true;
                        console.error(`[Music Menu] Scan failed: ${e.message}`);
                    }
                ...
                console.error(`[Music Menu] Could not launch scanner: ${e.message}`);
                button.set_sensitive(true);
