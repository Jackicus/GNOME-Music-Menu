...
// stamp too.
export default class MusicMenuExtension extends Extension {
    async enable() {
        ...
        try {
            ...
            if (this._enabling !== enabling)
                return;
            this._app = new module.MusicMenuApp(this);
            this._app.enable();
            console.log(`[Music Menu] Enabled from ${runDir}`);
        } catch (e) {
            console.error('[Music Menu] Failed to load lib/app.js:', e);
        }
    }
    ...
    disable() {
        ...
        if (this._app) {
            try {
                ...
                console.error('[Music Menu] Error during disable:', e);
            }
            this._app = null;
        }
    ...
    _stageLib() {
        const base = GLib.build_filenamev([GLib.get_user_runtime_dir(), 'music-menu']);
    ...
    _removeTree(file) {
        ...
        try {
            ...
            console.warn(`[Music Menu] Could not clean ${file.get_path()}: ${e.message}`);
        }
    }
}
