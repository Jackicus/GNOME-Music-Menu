...
```js
class MusicMenuLibraryIcon extends Dash.ShowAppsIcon {
    _init(gicon) {
        this._gicon = gicon;
        super._init();
...
try {
    ...
    console.warn(`[Music Menu] No button beside Show Apps: ${e}`);
    this._detach();
}
```
...
folding a workspace, and `id !== 0` is what spares it. Music Menu sets it
...
Music Menu needs a workspace held for as long as the library (or a pick)
...
**The one difference from Wallpaper Engine's copy.** Music Menu only ever
