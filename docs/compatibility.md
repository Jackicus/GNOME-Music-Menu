...
stages `lib/` into `$XDG_RUNTIME_DIR/music-menu/lib-<checksum>/` before
...
try {
    ...
    console.log('[Music Menu] libmanette is not installed; game controllers are not read.');
    return;
}
```
...
4. Install the zip rather than the development link: `make uninstall`, then
   `make pack`, then install the built `dist/music-menu@jackt.shell-extension.zip`
...
10. Disable and enable the extension ten times in a row and watch `make logs`
    ...
    `lib-<checksum>` directories under `$XDG_RUNTIME_DIR/music-menu/`
