...
4. deletes the temporary staging directory and reports
   `dist/music-menu@jackt.shell-extension.zip`.
...
prefs.js
stylesheet.css
schemas/org.gnome.shell.extensions.music-menu.gschema.xml
...
- **`backend/`** — a Python 3 program (`scan_library.py`, `media_scanner.py`,
  ...
  metadata online, and writes `~/.cache/music-menu/library.json`. It is
...
| Key | Now | Verdict |
|---|---|---|
| `uuid` | `music-menu@jackt` | Valid characters, not under `gnome.org`. Cannot change after the first upload — see [the name](#the-extension-name-versus-the-repo-name) |
...
`MusicMenuApp.disable()` (`lib/app.js`) removes the keybinding
...
`$XDG_RUNTIME_DIR/music-menu/lib-<stamp>/` on every `enable()` and
...
- `extension.js`'s `enable()` logs `[Music Menu] Enabled from ${runDir}`
  on every successful enable — not a failure path, the happy path.
- `lib/app.js`'s rebuild handler logs `[Music Menu] Rebuilt` every time
...
`~/.cache/music-menu/`. It is launched two ways, both already careful
...
The ID `org.gnome.shell.extensions.music-menu` and path
`/org/gnome/shell/extensions/music-menu/` use the required bases, the
...
followed it: both are "Video Menu". The UUID (`music-menu@jackt`) has
not, nor has the schema ID (`org.gnome.shell.extensions.music-menu`)
...
more than once pre-release. Decide whether to keep `music-menu@jackt`
...
1. **Decide the UUID.** The name is now "Video Menu"; the UUID is still
   `music-menu@jackt`. Keep it, or move the UUID, the schema ID and
...
10. **Test the packed zip, not the development link** — `make uninstall`,
    `make pack`, `gnome-extensions install dist/music-menu@jackt.shell-extension.zip`,
...
- **Web:** log in at https://extensions.gnome.org/upload/, choose
  `dist/music-menu@jackt.shell-extension.zip` (or whatever the UUID
