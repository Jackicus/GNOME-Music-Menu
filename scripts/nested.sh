#!/usr/bin/env bash
#
# Drive a throwaway nested GNOME Shell for testing Music Menu.
#
#   ./scripts/nested.sh start [WxH]   start a nested shell (default 1600x900) with
#                                     Music Menu ACTIVE, and open a live mirror window
...
# MUSIC_MENU_NESTED_IDLE seconds (default 600, 0 = never) without a command here,
...
RUN_DIR="${XDG_RUNTIME_DIR:-/tmp}/music-menu-nested"
...
IDLE_SECS="${MUSIC_MENU_NESTED_IDLE:-600}"
...
cmd_start() {
    ...
    if nested_env gsettings get org.gnome.shell enabled-extensions 2>/dev/null | grep -qF "'$UUID'"; then
        wait_state ACTIVE \
            || die "Music Menu is $(nested_state) after startup -- check './scripts/nested.sh logs' for a JS error."
        ok "Music Menu ACTIVE."
    else
        enable_in_nested
    fi
...
# is about Music Menu, and it buries the lines that are.
filtered_log() {
